...
goto endofperl
@rem ';
#!"C:\xampp_RailoPortable2\perl\bin\perl.exe"
#line 15
use strict;
use warnings;
use Getopt::Long;
...
my ${CC} = q[cl];
my ${LIBS} = q[];
my ${installbuilddir} = q[C:\xampp_RailoPortable2\apache\build];
my ${APRUTIL_LIB_TARGET} = q[];
my ${bindir} = q[C:\xampp_RailoPortable2\apache\bin];
my ${APRUTIL_SO_EXT} = q[dll];
my ${LD} = q[link];
my ${CPP} = q[cl -nologo -E];
my ${LDFLAGS} = q[ kernel32.lib /nologo /subsystem:windows /dll /machine:I386 ];
my ${includedir} = q[C:\xampp_RailoPortable2\apache\include];
my ${exec_prefix} = q[C:\xampp_RailoPortable2\apache];
my ${datadir} = q[C:\xampp_RailoPortable2\apache];
my ${APRUTIL_LIBNAME} = q[libaprutil-1.lib];
my ${libdir} = q[C:\xampp_RailoPortable2\apache\lib];
...
my ${prefix} = q[C:\xampp_RailoPortable2\apache];
